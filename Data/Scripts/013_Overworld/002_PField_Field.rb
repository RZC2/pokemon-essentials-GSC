...
def pbCheckAllFainted
  if pbAllFainted
    pbMessage(_INTL("{1} is out of useable POKéMON!\1",$Trainer.name))
    pbMessage(_INTL("{1} whited out!",$Trainer.name))
    pbBGMFade(1.0)
    pbBGSFade(1.0)
...
def pbItemBall(item,quantity=1)
  ...
  if $PokemonBag.pbStoreItem(item,quantity)   # If item can be picked up
    ...
    if isConst?(item,PBItems,:LEFTOVERS)
      pbMessage(_INTL("\\me[{1}]{2} found some {3}!\\wtnp[30]",meName,$Trainer.name,itemname))
    elsif pbIsTechnicalMachine?(item)   # TM
      pbMessage(_INTL("\\me[{1}]{2} found TM{3}!\\wtnp[30]",meName,$Trainer.name,itemname))
	elsif pbIsHiddenMachine?(item)      # HM
	  pbMessage(_INTL("\\me[{1}]{2} found {3}!\\wtnp[30]",meName,$Trainer.name,itemname))
    elsif quantity>1
      pbMessage(_INTL("\\me[{1}]{2} found {3} x{4}!\\wtnp[30]",meName,$Trainer.name,itemname,quantity))
	...
	if !pbIsMachine?(item)
      pbMessage(_INTL("{1} put the {2} in the {3} POCKET.",
        $Trainer.name,itemname,PokemonBag.pocketNames()[pocket]))
	elsif pbIsTechnicalMachine?(item)
	  pbMessage(_INTL("{1} put the TM in the TM POCKET.",$Trainer.name))
	elsif pbIsHiddenMachine?(item)
	  pbMessage(_INTL("{1} put the HM in the TM POCKET.",$Trainer.name))
	end
    return true
  end
  # Can't add the item
  if isConst?(item,PBItems,:LEFTOVERS)
    pbMessage(_INTL("{1}found some {2}!\\wtnp[30]",$Trainer.name,itemname))
  elsif pbIsTechnicalMachine?(item)   # TM
    pbMessage(_INTL("{1} found TM{2}!\\wtnp[30]",$Trainer.name,itemname))
  elsif pbIsHiddenMachine?(item)   # HM
    pbMessage(_INTL("{1} found {2}!\\wtnp[30]",$Trainer.name,itemname))
  elsif quantity>1
    pbMessage(_INTL("{1} found {2} x{3}!\\wtnp[30]",$Trainer.name,itemname,quantity))
  elsif itemname.starts_with_vowel?
    pbMessage(_INTL("{1} found {2}!\\wtnp[30]",$Trainer.name,itemname))
  else
    pbMessage(_INTL("{1} found {2}!\\wtnp[30]",$Trainer.name,itemname))
...
def pbReceiveItem(item,quantity=1)
  ...
  if isConst?(item,PBItems,:LEFTOVERS)
    pbMessage(_INTL("\\me[{1}]{2} received some {3}.\\wtnp[30]",meName,$Trainer.name,itemname))
  elsif pbIsTechnicalMachine?(item)   # TM
    pbMessage(_INTL("\\me[{1}]{2} received TM{3}.\\wtnp[48]",meName,$Trainer.name,itemname))
  elsif pbIsHiddenMachine?(item)      # HM
    pbMessage(_INTL("\\me[{1}]{2} received {3}.\\wtnp[48]",meName,$Trainer.name,itemname))
  elsif quantity>1
    pbMessage(_INTL("\\me[{1}]{2} received {3} x{4}.\\wtnp[30]",meName,$Trainer.name,itemname,quantity))
  elsif itemname.starts_with_vowel?
    pbMessage(_INTL("\\me[{1}]{2} received {3}.\\wtnp[30]",meName,$Trainer.name,itemname))
  else
    pbMessage(_INTL("\\me[{1}]{2} received {3}.\\wtnp[30]",meName,$Trainer.name,itemname))
  end
