...
def pbRandomPokemonFromRule(rule,trainer)
  ...
  loop do
    ...
    itemlist=[
       ...
       :BLACKSLUDGE,:BRIGHTPOWDER,:MINTBERRY,:CHOICEBAND,
       ...
       :LIGHTCLAY,:MIRACLEBERRY,:OCCABERRY,:PETAYABERRY,:SALACBERRY,
    ...
    if level<10
      item=(getConst(PBItems,:BERRY) || item) if rand(40)==0 ||
            isConst?(item,PBItems,:GOLDBERRY)
    end
    if level>20
      item=(getConst(PBItems,:GOLDBERRY) || item) if rand(40)==0 ||
            isConst?(item,PBItems,:BERRY)
    end
    ...
    if moves.include?((getConst(PBMoves,:REST) || -1))
       item=getID(PBItems,:MIRACLEBERRY) if rand(3)==0
       item=getID(PBItems,:MINTBERRY) if rand(4)==0
    end
