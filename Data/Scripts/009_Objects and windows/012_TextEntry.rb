  ...
  def pbStartScene(helptext,minlength,maxlength,initialText,subject=0,pokemon=nil)
    ...
    when 2   # Pokémon
      if pokemon
        ...
        textpos=[]
        if pokemon.male?
          textpos.push([_INTL("♂"),0,0,false,Color.new(0,0,255),Color.new(255,255,255)])
        elsif pokemon.female?
          textpos.push([_INTL("♀"),0,0,false,Color.new(255,0,0),Color.new(255,255,255)])
        end
...
class PokemonEntryScene2
  @@Characters=[
     [("ABCDEFGHI"+"JKLMNOPQR"+"STUVWXYZ "+"-?!/.,   ").scan(/./),_INTL("UPPER")],
     [("abcdefghi"+"jklmnopqr"+"stuvwxyz "+"×():;[]  ").scan(/./),_INTL("lower")]
  ]
  ROWS    = 9
  COLUMNS = 4
  MODE    = -3
  ...
  def pbStartScene(helptext,minlength,maxlength,initialText,subject=0,pokemon=nil)
    ...
    @viewport=Viewport.new(0,0,Graphics.width,Graphics.height)
    @viewport.z=99999
    @helptext=[helptext[0,13],helptext[13,13]]
    ...
    for i in 0...2
      ...
      for y in 0...COLUMNS
        for x in 0...ROWS
          textPos.push([@@Characters[i][0][pos],32+x*32,120+y*32,0,
             MessageConfig::LIGHTTEXTBASE])
          pos+=1
        end
      end
      modeswap=[_INTL("lower"),_INTL("UPPER")]
      textPos.push([modeswap[i],32,248,0,MessageConfig::LIGHTTEXTBASE])
      textPos.push([_INTL("DEL"),144,248,0,MessageConfig::LIGHTTEXTBASE])
      textPos.push([_INTL("END"),240,248,0,MessageConfig::LIGHTTEXTBASE])
      pbDrawTextPositions(@bitmaps[i+2],textPos)
    end
    @bitmaps[4]=BitmapWrapper.new(16,4)
    @bitmaps[4].fill_rect(2,0,16,4,Color.new(0,0,0))
    ...
    when 1   # Player
      if $PokemonGlobal
        ...
        if meta
          ...
          #charwidth=@sprites["subject"].bitmap.width
          #charheight=@sprites["subject"].bitmap.height
          @sprites["subject"].x = 16*2# - charwidth/4
          @sprites["subject"].y = 12*2# - charheight/4
        end
      end
    when 2   # Pokémon
      if pokemon
        @sprites["subject"]=PokemonIconSprite.new(pokemon,@viewport)
        @sprites["subject"].x=16*2
        @sprites["subject"].y=12*2
        @sprites["gender"]=BitmapSprite.new(32,32,@viewport)
        @sprites["gender"].x=16
        @sprites["gender"].y=32
        @sprites["gender"].bitmap.clear
    ...
    @maxlength.times {|i|
       ...
       @sprites["blank#{i}"].bitmap=@bitmaps[4]
       @sprites["blank#{i}"].x=80+16*i
       @blanks[i]=0
    }
    @sprites["bottomtab"]=SpriteWrapper.new(@viewport)
    @sprites["bottomtab"].x=0
    @sprites["bottomtab"].y=0
    @sprites["bottomtab"].bitmap=@bitmaps[0+2]
    @init=true
  ...
  def pbDoUpdateOverlay
    ...
    textPositions=[
      [@helptext[0],80,24,false,MessageConfig::LIGHTTEXTBASE]
    ]
    textPositions.push([@helptext[1],80,56,false,MessageConfig::LIGHTTEXTBASE]) if @helptext[1]
    chars=@helper.textChars
    x=80
    for ch in chars
      textPositions.push([ch,x,88,false,MessageConfig::LIGHTTEXTBASE])
      x+=16
    end
  ...
  def pbMoveCursor
    ...
    if Input.repeat?(Input::LEFT)
      ...
      if @cursorpos<0 # Controls
        ...
        when BACK; @cursorpos=ROWS*(COLUMNS-1)+3
        when OK; @cursorpos=ROWS*(COLUMNS-1)+6
        end
      elsif @cursorpos<ROWS # Top row of letters
        case @cursorpos
        when 0,1,2; @cursorpos=MODE
        when 3,4,5; @cursorpos=BACK
        when 6,7,8; @cursorpos=OK
        end
      else
      ...
      if @cursorpos<0 # Controls
        ...
        when BACK; @cursorpos=3
        when OK; @cursorpos=6
        end
      elsif @cursorpos>=ROWS*(COLUMNS-1) # Bottom row of letters
        case @cursorpos
        when ROWS*(COLUMNS-1),ROWS*(COLUMNS-1)+1,ROWS*(COLUMNS-1)+2
          @cursorpos=MODE
        when ROWS*(COLUMNS-1)+3,ROWS*(COLUMNS-1)+4,ROWS*(COLUMNS-1)+5
          @cursorpos=BACK
        when ROWS*(COLUMNS-1)+6,ROWS*(COLUMNS-1)+7,ROWS*(COLUMNS-1)+8
          @cursorpos=OK
        end
      else
        cursordiv=wrapmod((cursordiv+1),COLUMNS)
