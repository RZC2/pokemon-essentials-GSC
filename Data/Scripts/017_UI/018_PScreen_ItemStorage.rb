...
class ItemStorage_Scene
  ...
  TITLEBASECOLOR      = Color.new(0,0,0)
  TITLESHADOWCOLOR    = Color.new(0,0,0,0)
  ITEMSVISIBLE        = 7
  ...
  def pbStartScene(bag)
    ...
    # Item list
    @sprites["itemwindow"] = Window_PokemonItemStorage.new(@bag,0,14,317,32+ITEMSVISIBLE*32)
    ...
    # Item description
    @sprites["itemtextwindow"] = Window_UnformattedTextPokemon.newWithSize("",0,190,Graphics.width-0,190,@viewport)
  ...
  def pbRefresh
    ...
    if itemwindow.item==0
      @sprites["itemtextwindow"].text = _INTL("")
    else
