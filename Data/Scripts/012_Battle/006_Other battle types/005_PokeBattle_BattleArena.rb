...
class PokeBattle_Scene
  ...
  def updateJudgment(window,phase,battler1,battler2,ratings1,ratings2)
	...
	colorBase = Color.new(0,0,0)
	colorShadow = Color.new(255,255,255)
    window.contents.clear
    pbSetSystemFont(window.contents)
    textpos = [
       [_INTL("PLAYER's"),0,0,0,Color.new(255,0,0),colorShadow],
       [_INTL("VS"),144,0,2,colorBase,colorShadow],
       [_INTL("RIVAL's"),172,0,0,colorBase,colorShadow],
       [_INTL("Mind"),141,25,2,colorBase,colorShadow],
       [_INTL("Skill"),145,57,2,colorBase,colorShadow],
       [_INTL("Body"),141,89,2,colorBase,colorShadow],
       [sprintf("%d",total1),64,130,2,colorBase,colorShadow],
       [_INTL("Judgment"),145,130,2,colorBase,colorShadow],
       [sprintf("%d",total2),226,130,2,colorBase,colorShadow]
    ]
    ...
    for i in 0...phase
      ...
      images.push(["Graphics/Pictures/judgment",64-16,y,x,0,16,16])
      x = (ratings1[i]==ratings2[i]) ? 64 : ((ratings1[i]<ratings2[i]) ? 0 : 32)
      images.push(["Graphics/Pictures/judgment",240-16,y,x,0,16,16])
    end
    pbDrawImagePositions(window.contents,images)
    window.contents.fill_rect(16,125,256,4,colorBase)
  end
  ...
  def pbBattleArenaJudgment(battler1,battler2,ratings1,ratings2)
    ...
    begin
      ...
      dimmingvp.z = 99999
      infowindow = SpriteWindow_Base.new(0,0,320,192)
