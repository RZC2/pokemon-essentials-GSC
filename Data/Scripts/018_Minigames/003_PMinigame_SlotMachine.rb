...
class SlotMachineScene
  ...
  def pbPayout
    ...
    for i in 0...combinations.length
      ...
      wonRow[i]=true
      case combinations[i]
      when [1,1,1]   # Three Pokéballs
        payout += 8
      when [2,2,2]   # Three Staryus
        payout += 8
      when [3,3,3]   # Three Pikachus
        payout += 15
      when [4,4,4]   # Three Squirtles
        payout += 15
	  when [5,5,5], [6,6,6]   # Red 777, Blue 777
        payout += 300
        bonus = 2 if bonus<2
      when [5,5,6], [6,6,5]   # 777, red red blue, blue blue red
        payout += 90
        bonus = 1 if bonus<1
    ...
    if payout>0 || @replay
      ...
      until frame==Graphics.frame_rate*3
        ...
        @sprites["window1"].src_rect.set(320*((frame/timePerFrame)%4),0,320,96)
        if bonus>0
          @sprites["window2"].setBitmap(sprintf("Graphics/Pictures/Slot Machine/bonus"))
          @sprites["window2"].src_rect.set(320*(bonus-1),0,320,96)
        end
        @sprites["light1"].visible=true
        @sprites["light1"].src_rect.set(0,178*((frame/timePerFrame)%4),320,178)
        @sprites["light2"].visible=true
        @sprites["light2"].src_rect.set(0,178*((frame/timePerFrame)%4),320,178)
        for i in 1..5
          if wonRow[i-1]
      ...
      until frame==Graphics.frame_rate*2
        ...
        @sprites["window1"].src_rect.set(320*((frame/timePerFrame)%2),0,320,96)
        frame += 1
      end
    end
    @wager = 0
  ...
  def pbStartScene(difficulty)
    ...
    @sprites["reel1"]=SlotMachineReel.new(80,64,difficulty)
    @sprites["reel2"]=SlotMachineReel.new(144,64,difficulty)
    @sprites["reel3"]=SlotMachineReel.new(208,64,difficulty)
    for i in 1..3
    ...
    @sprites["window1"].src_rect.set(0,0,320,96)
    @sprites["window2"]=IconSprite.new(0,192,@viewport)
    @sprites["credit"]=SlotMachineScore.new(64,16,$PokemonGlobal.coins)
  ...
  def pbMain
    ...
    loop do
      ...
      if @sprites["credit"].score==MAX_COINS
        ...
        @sprites["window1"].src_rect.set(320*((frame/spinFrameTime)%4),0,320,96)
        ...
        @sprites["window1"].setBitmap(sprintf("Graphics/Pictures/Slot Machine/insert"))
        @sprites["window1"].src_rect.set(320*((frame/insertFrameTime)%2),0,320,96)
        if @wager>0
          @sprites["window2"].setBitmap(sprintf("Graphics/Pictures/Slot Machine/press"))
          @sprites["window2"].src_rect.set(320*((frame/insertFrameTime)%2),0,320,96)
        end
