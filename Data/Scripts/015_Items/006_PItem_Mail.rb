...
def pbDisplayMail(mail,_bearer=nil)
  ...
  baseForDarkBG    = Color.new(0,0,0)
  shadowForDarkBG  = Color.new(255,255,255,0)
  baseForLightBG   = Color.new(0,0,0)
  shadowForLightBG = Color.new(255,255,255,0)
  if mail.message && mail.message!=""
    isDark = isDarkBackground(sprites["card"].bitmap,Rect.new(48,48,Graphics.width-96,32*7))
    drawTextEx(overlay,32,101,Graphics.width-52,2,mail.message,
       (isDark) ? baseForDarkBG : baseForLightBG,
       (isDark) ? shadowForDarkBG : shadowForLightBG)
  end
  if mail.sender && mail.sender!=""
    isDark = isDarkBackground(sprites["card"].bitmap,Rect.new(256,224,64,0))
    drawTextEx(overlay,80,213,140,0,mail.sender,
       (isDark) ? baseForDarkBG : baseForLightBG,
       (isDark) ? shadowForDarkBG : shadowForLightBG)
  end
